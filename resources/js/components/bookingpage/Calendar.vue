<template>
    <div>
        <div class="d-flex text-center align-items-center" style="max-width:900px;margin: auto">
            <div>
                <v-btn @click="previousWeek" :disabled="ready || dateIndex==0" v-if="windowSizeX>480">Back</v-btn>
            </div>
                                ...
                                <div v-for="(c, i) in calendar" :key="i">
                                    ...
                                    <div style="height: 31px" class="p-1" v-for="(time, j) in c" :key="j">
                                        <!-- <router-link style="text-decoration: none;" :to="{name: 'confirmation', $route, query:{date: time.date, duration: duration, title: title, price: price}}" :class="{ 'is-disabled': time.isBooking }"> -->
                                            <b @click="select(time)">{{ (time.isBooking==false) ? '〇' : '×' }}</b>
                                        <!-- </router-link> -->
                                    </div>
                                </div>
                            </div>
            ...
            <div>
                <v-btn @click="nextWeek" :disabled="ready || dateIndex==1" v-if="windowSizeX>480">Next</v-btn>
            </div>
        </div>
        <div class="justify-content-center d-flex" style="width: 100%;" v-if="windowSizeX<480">
            <v-btn class="mx-4" @click="previousWeek" :disabled="ready || dateIndex==0">Back</v-btn>
            <v-btn class="mx-4" @click="nextWeek" :disabled="ready || dateIndex==1">Next</v-btn>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        select(time){
            const from = time.date;
            const to = from + this.duration*60*1000;
            const params = {from: from, to: to}
            this.$emit('time', params);
        },
        makeCalendar(index){
            this.calendar = [];
